...
class MovieService {
  ...
  Future<List<Comment>> getComments(int movieId) async {
    final res = await ApiClient.get('/comments.php', params: {'movie_id': movieId.toString()});
    final data = res.data;
  ...
  /// Post a comment — returns {success, message}
  Future<Map<String, dynamic>> postComment(int movieId, String content, {int? rating, bool spoiler = false, int? parentId}) async {
    final res = await ApiClient.post('/comments.php', data: {
      'movie_id': movieId,
      'content': content,
      'spoiler': spoiler,
