...
class _WatchRoomScreenState extends State<WatchRoomScreen> with WidgetsBindingObserver {
  ...
  void _initPlayer() {
    ...
    _player!.stream.position.listen((pos) {
      ...
      // Throttle: chỉ update UI mỗi 500ms
  ...
  Future<void> _loadVideoSource(String m3u8Url) async {
    ...
    try {
      ...
      // Không auto-play — đợi sync từ server
    } catch (e) {
  ...
  Future<void> _updateHostState(String state) async {
    if (!_isHost) return;
    _videoState = state;
    _videoTime = _player?.state.position.inSeconds.toDouble() ?? 0;
  ...
  void _applyServerState(Map<String, dynamic> data) {
    ...
    if (newState == 'playing') {
      ...
      if (diff > 10) {
        // Lệch nhiều → seek + play
        _seekWhenReady(newTime);
      } else if (!_player!.state.playing) {
        // Lệch ít + đang pause → play ngay
        _setLocalAction();
        _player!.play();
      }
    } else {
  ...
  void _seekWhenReady(double targetTime) {
    ...
    // Seek
    _player!.seek(Duration(seconds: targetTime.toInt())).then((_) {
      _seekRetryTimer?.cancel(); // Cancel fallback nếu seek thành công
      // Đợi 800ms để audio/video sync hoàn toàn
      Future.delayed(const Duration(milliseconds: 800), () {
        if (!mounted || _player == null) return;
        // Verify seek thành công — nếu chưa đúng thì seek lại
        final actual = _player!.state.position.inSeconds;
        final diff = (actual - targetTime).abs();
        if (diff > 5) {
          _player!.seek(Duration(seconds: targetTime.toInt()));
        }
        // Restore volume theo user setting + play
  ...
  /// Host sync video_time mỗi 5 giây — guest cần biết vị trí real-time
  void _startHostSync() {
    _hostSyncTimer?.cancel();
  ...
  Widget build(BuildContext context) {
    ...
    if (orientation == Orientation.landscape && !_isFullscreen) {
      ...
      // Không gọi setPreferredOrientations → cho phép tự do xoay
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        onTap: () {
                          ...
                          _updateHostState(_videoState);
                          _saveWatchtime();
                          if (!_showControls) { setState(() {}); _autoHideControls(); }
                        },
                        onDoubleTap: () {
                          final pos = _player?.state.position ?? Duration.zero;
                          _setLocalAction();
  ...
  void _toggleFullscreen() {
    ...
    if (_isFullscreen) {
      ...
      // Bấm back → unlock tất cả orientation (cho phép tự do xoay)
      SystemChrome.setPreferredOrientations([
        DeviceOrientation.portraitUp,
        DeviceOrientation.landscapeLeft,
        DeviceOrientation.landscapeRight,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoCard('Thông tin phòng', [
            ...
            if (_epName.isNotEmpty) _infoRow('Tập', 'Tập $_epName'),
